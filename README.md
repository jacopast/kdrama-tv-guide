...
- **2026-09-02**: Netflix 공식 작품 페이지와 Tudum 가을 영화 라인업을 재검증해 `크로스 2 (Mission: Cross 2)`의 `2026-09-01` 임시 공개 표기를 제거. 공식 표기는 2026년 가을이며 정확한 날짜가 아직 없으므로, 날짜가 확정될 때까지 가시 편성표에서는 제외하고 `REFRESH_GUIDE.md` 확인 대기 목록에서 추적.
- **2026-09-02**: 편성표는 텍스트 중심으로 유지하고, 제목을 클릭해 여는 상세 정보 모달에 검증된 작품별 포스터를 표시하도록 개선. 포스터 소스가 아직 검증되지 않은 작품은 임의의 공용 이미지를 쓰지 않고 포스터 영역을 숨김.
...
- **2026-08-19 [Claude/Cowork] (22차)**: "청부살인자의 가게 시즌2"(영어 제목을 제가 직역해 지어낸 오타)를 실제 정식 제목 "킬러들의 쇼핑몰 시즌2"로 수정. 같은 실수 재발 방지 차 전체 제목을 재검증(결혼의 완성/동궁은 이미 정확했음, "크로스 2"에 임의로 붙인 "(미션 크로스 2)" 괄호는 제거). REFRESH_GUIDE.md에 "한글 제목은 영어에서 직역/추측 금지, 반드시 위키백과·나무위키·공식 뉴스룸에서 정식 제목 그대로 확인" 원칙 명시.
